...
import com.zhlw.azurereader.ui.read.ReadPresenter;
...
public class DialogCreator {
    ...
    public static Dialog createReadDetailSetting(final Context context, final Setting setting,
        ...
        //自动滚屏速度
        SeekBar sbScrollSpeed = (SeekBar)view.findViewById(R.id.sb_auto_scroll_progress);//自动滚动速度的
    ...
    public static Dialog createReadSetting(final Context context, final boolean isDayStyle, int chapterProgress,
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            window.setStatusBarColor(dialog.getContext().getResources().getColor(R.color.colorPrimaryDark));
        }
        ...
        if (!isDayStyle) {
            ivNightAndDay.setImageResource(R.mipmap.sun_bigger);
            tvNightAndDay.setText(context.getString(R.string.day));
        }

        Log.d("debug", "createReadSetting: ..............init only once ");
        //创建viewmodel
        ReadActivity mReadActivity = (ReadActivity) context;
        MyViewModel viewModel = mReadActivity.getViewModel();
        viewModel.getmProgress().observe((ReadActivity)context, new Observer<Integer>() {
            @Override
            public void onChanged(Integer integer) {
                // 当数值发生改变时要执行的操作
                Log.d("debug", "onChanged: .................." + integer);
                sbChapterProgress.setProgress(integer);
            }
        });
        ...
        llNightAndDay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (tvNightAndDay.getText().toString().equals(context.getString(R.string.day))) {
                    isDay = false;
                    ivNightAndDay.setImageResource(R.mipmap.day_light_switch);
